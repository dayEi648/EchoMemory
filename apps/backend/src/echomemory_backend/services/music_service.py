...
def _set_music_authors(db: Session, music: Music, author_ids: list[int]) -> None:
    """设置音乐的作者关联，覆盖原有作者。"""
    db.query(MusicAuthor).filter(MusicAuthor.music_id == music.id).delete(
...
def _set_music_instruments(
    db: Session, music: Music, instrument_ids: list[int]
) -> None:
    """设置音乐的乐器关联，覆盖原有乐器。"""
    db.query(MusicInstrument).filter(
...
def _set_music_emotion_tags(
    db: Session, music: Music, tag_ids: list[int]
) -> None:
    """设置音乐的情绪标签关联，覆盖原有标签。"""
    db.query(MusicEmotionTag).filter(
...
def _set_music_interest_tags(
    db: Session, music: Music, tag_ids: list[int]
) -> None:
    """设置音乐的兴趣标签关联，覆盖原有标签。"""
    db.query(MusicInterestTag).filter(
...
def create_music(
    ...
    *,
    ...
    db.add(music)
    db.flush()  # 获取 music.id

    if author_ids:
        _set_music_authors(db, music, author_ids)
    if instrument_ids:
        _set_music_instruments(db, music, instrument_ids)
    if emotion_tag_ids:
        _set_music_emotion_tags(db, music, emotion_tag_ids)
    if interest_tag_ids:
        _set_music_interest_tags(db, music, interest_tag_ids)
...
def search_musics(
    ...
    *,
    ...
    if q:
        stmt = stmt.where(Music.title.ilike(f"%{q}%"))
...
def set_music_published(db: Session, music: Music, published: bool) -> Music:
    """设置音乐上架/下架状态。"""
